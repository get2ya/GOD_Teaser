...
**절대 하면 안 되는 방식 (깜빡임 발생):**
- A영상 재생 중 B영상을 `opacity: 0`으로 숨겨두기
- A영상 종료 순간 A는 `display: none`, B는 `opacity: 1`로 동시 전환
- transition 없이 즉시 전환

위 방식은 A→B 전환 시 깜빡임이 발생함.

**올바른 방식 (Seamless 전환):**
- A영상(z-index:2)과 B영상(z-index:1) 둘 다 동시에 재생
- B영상은 뒤 레이어에서 처음부터 `opacity: 1`로 보이는 상태
- A영상 종료 직전(0.033초 전)에 B영상 시작
- A영상 종료 시 A만 숨김 → B가 자연스럽게 보임
- 두 영상이 겹쳐있어 전환 시 끊김 없음
